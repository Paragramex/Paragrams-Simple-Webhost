...
define('_CONFIG', __DIR__.'/.usersettings');
$phpVer = phpversion();
$safesites = array("webhost.paragram.repl.co", "paragram.rf.gd");
if( file_exists(_CONFIG) ){
	...
	$phpInfo['Using a Safe Server'] = (in_array($_SERVER['HTTP_HOST'], $safesites)) ? '<small>You&#39;re ready deploy securely!</small><br/><svg viewBox="0 0 24 24"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z" class="green"/></svg>' : '<small>This site is not a trusted site by the developers, use at your own risk!</small><br/><svg viewBox="0 0 24 24"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z" class="red"/></svg>';
	html_setup($phpInfo);
	exit;
}
...
if( !isset($_SESSION['__allowed']) && strlen($config->password) ) {
	...
	$label = isset($label) ? $label : '<label for="auth">User Panel v'.VERSION.'</label>';
	html_login($label);
	exit;
}
...
$welcome = @$_SESSION['gitJSON']['latest_ver'] != VERSION ? 'New Update Available, Upgrade Now' : 'Logged In Successfully, Enjoy Your Hosting!';
...
<head>
	<title>User Panel v<?= VERSION; ?></title>
	<meta charset="utf-8">
...
<body onload="toast('<?= @$welcome; ?>');">
	...
	<footer>
		<div>
			<?php if( @$_SESSION['gitJSON']['latest_ver'] == VERSION ) :?><span>User Panel v<?= VERSION; ?></span><?php else : ?><a class="btn upgrade">New Update Available</a><?php endif; ?>
<b> &nbsp; &bull; &nbsp;</b>
			<span>Hosting Software Made By &nbsp;<a target="_blank" href="https://replit.com/@paragram">Paragram</a></span>
		</div>
	</footer>
		...
		(function($){
			...
			*******************************************/
			...
			function docTitle(str = null){
				document.title = typeof str == 'string' ? str : 'User Panel v' + VERSION;
			}
...
<body>
	<main>
		<form method="POST" onsubmit="document.querySelector('main').style.opacity = 0; document.querySelector('body').style.backgroundColor = '#035';">
			...
			<label>Password (Pre-Filled)<input type="text" name="pwd" placeholder="Enter the Password" value="<?php echo $_SESSION["nonhash"] ?>"></label>
			<button name="do" value="install">Complete Registration</button>
		</form>
	</main>
</body>
</html>
...
function html_login($label){?>
	...
	# Version: <?= VERSION; ?>
	# 
	# https://webhost.paragram.repl.co
	# Register now for a free website hosted by paragram!!!
	=============================================-->
	<!DOCTYPE html>
	<html lang="en">
	<head>
...
<body>
	...
	<footer>
		<div>
			<span>User Panel v<?= VERSION; ?></span>
			<b> &nbsp; &bull; &nbsp;</b>
			<span>Hosting Software Made By &nbsp;<a target="_blank" href="https://replit.com/@paragram">Paragram</a></span>
		</div>
	</footer>
